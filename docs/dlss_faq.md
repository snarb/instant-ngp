...
## How do I get the highest DLSS image quality?
1. **Target your final resolution first.** Resize the GUI window or pass `--width/--height` to `scripts/run.py` so the DLSS output surface matches your intended capture size; DLSS always upscales into that fixed `full_resolution` buffer.【F:src/testbed.cu†L3256-L3307】【F:scripts/run.py†L392-L409】
2. **Pick a high-fidelity preset.** Use the GUI drop-down or `--dlss_mode max_quality`, `--dlss_mode ultra_quality`, or `--dlss_mode dlaa` to force the best mode your NGX build exposes; DLAA turns off dynamic resolution and keeps the DLSS input the same size as the output for anti-aliasing-only runs.【F:src/testbed.cu†L5697-L5718】【F:scripts/run.py†L158-L175】
3. **Lock input resolution when detail matters.** Disable dynamic resolution (or keep DLAA selected) so the renderer holds `in_resolution == full_resolution` instead of downscaling frames to chase frame time; this prevents DLSS from seeing undersized inputs when performance fluctuates.【F:src/testbed.cu†L3333-L3374】【F:src/render_buffer.cu†L600-L672】
4. **Tune sharpening for the scene.** Adjust the DLSS sharpening slider or pass `--dlss_sharpening <0-1>`; values near 1.0 mirror the GUI’s default recommendation and can counter soft edges after upscaling.【F:src/testbed.cu†L1403-L1435】【F:scripts/run.py†L69-L181】
